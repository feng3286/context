import React from 'react';
import { Trans, useTranslation } from 'react-i18next';
import { Input } from '@renderer/lib/ui/input';

interface Props {
  site: string;
  email: string;
  token: string;
  onChange: (update: Partial<{ site: string; email: string; token: string }>) => void;
  error?: string | null;
}

const JiraSetupForm: React.FC<Props> = ({ site, email, token, onChange, error }) => {
  const { t } = useTranslation();
  return (
    <div className="grid gap-2">
      <Input
        placeholder={t('integrations:jira.siteUrlPlaceholder')}
        value={site}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ site: e.target.value })}
        className="h-9 w-full"
        autoFocus
      />
      <Input
        placeholder={t('integrations:jira.emailPlaceholder')}
        value={email}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ email: e.target.value })}
        className="h-9 w-full"
      />
      <Input
        type="password"
        placeholder={t('integrations:jira.apiTokenPlaceholder')}
        value={token}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange({ token: e.target.value })}
        className="h-9 w-full"
      />
      <p className="text-xs text-muted-foreground">
        <Trans
          t={t}
          i18nKey="integrations:jira.createTokenHint"
          components={{ url: <span className="font-medium" /> }}
        />
      </p>
      {error ? (
        <p className="text-xs text-destructive" role="alert">
          {error}
        </p>
      ) : null}
    </div>
  );
};

export default JiraSetupForm;
