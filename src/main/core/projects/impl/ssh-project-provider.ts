...
export class SshProjectProvider implements ProjectProvider {
  ...
  async openTask(
    ...
    const promise = withTimeout(
      ...
      .catch((e) => {
        ...
        this.bootstrapErrors.set(task.id, provisionError);
        this.provisioningTasks.delete(task.id);
        log.error('SshProjectProvider: failed to open task', {
          taskId: task.id,
          error: String(e),
        });
        return err(provisionError);
  ...
  private async doOpenTask(
    ...
    log.debug('SshProjectProvider: doOpenTask START', {
      taskId: task.id,
      workDir,
    });
    ...
    try {
      ...
      log.debug('SshProjectProvider: doOpenTask DONE', {
        taskId: task.id,
      });
  ...
  async listWorktreeBranches(): Promise<string[]> {
    return this.worktreeService.listWorktreeBranches();
  }
  ...
  async validateWorktreeBranch(
    ...
    try {
      await this.fs.stat(worktreePath);
    } catch {
