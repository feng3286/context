...
describe('getTaskEnvVars', () => {
  ...
  it('returns all six env vars', () => {
    ...
    expect(vars).toHaveProperty('CONTEXT_TASK_ID');
    expect(vars).toHaveProperty('CONTEXT_TASK_NAME');
    expect(vars).toHaveProperty('CONTEXT_TASK_PATH');
    expect(vars).toHaveProperty('CONTEXT_ROOT_PATH');
    expect(vars).toHaveProperty('CONTEXT_DEFAULT_BRANCH');
    expect(vars).toHaveProperty('CONTEXT_PORT');
  });

  it('passes through taskId, taskPath, projectPath directly', () => {
    const vars = getTaskEnvVars(baseCtx);
    expect(vars.CONTEXT_TASK_ID).toBe('task-abc-123');
    expect(vars.CONTEXT_TASK_PATH).toBe('/home/user/worktrees/fix-login-bug');
    expect(vars.CONTEXT_ROOT_PATH).toBe('/home/user/projects/myapp');
  });

  it('slugifies the task name', () => {
    const vars = getTaskEnvVars(baseCtx);
    expect(vars.CONTEXT_TASK_NAME).toBe('fix-login-bug');
  });

  it('strips leading/trailing hyphens from slug', () => {
    const vars = getTaskEnvVars({ ...baseCtx, taskName: '---Hello World---' });
    expect(vars.CONTEXT_TASK_NAME).toBe('hello-world');
  });

  it('falls back to "task" when name slugifies to empty', () => {
    const vars = getTaskEnvVars({ ...baseCtx, taskName: '!!!' });
    expect(vars.CONTEXT_TASK_NAME).toBe('task');
  });

  it('defaults defaultBranch to "main"', () => {
    const { defaultBranch: _, ...ctx } = baseCtx;
    const vars = getTaskEnvVars(ctx as TaskEnvContext);
    expect(vars.CONTEXT_DEFAULT_BRANCH).toBe('main');
  });

  it('uses provided defaultBranch', () => {
    const vars = getTaskEnvVars({ ...baseCtx, defaultBranch: 'develop' });
    expect(vars.CONTEXT_DEFAULT_BRANCH).toBe('develop');
  });

  it('generates a port in the 50000-59990 range', () => {
    const vars = getTaskEnvVars(baseCtx);
    const port = Number(vars.CONTEXT_PORT);
  ...
  it('generates stable port for same input', () => {
    ...
    expect(a.CONTEXT_PORT).toBe(b.CONTEXT_PORT);
  });

  it('uses portSeed over taskPath when provided', () => {
    const withSeed = getTaskEnvVars({ ...baseCtx, portSeed: 'custom-seed' });
    expect(typeof withSeed.CONTEXT_PORT).toBe('string');
    expect(Number(withSeed.CONTEXT_PORT)).toBeGreaterThanOrEqual(50000);
  });
});
